document.getElementById('logo-link').addEventListener('click', (e) => {
    ...
    if (window.location.pathname.endsWith('index.html') || window.location.pathname === '/') {
        e.preventDefault(); // If already on homepage, prevent default link behavior (avoids unnecessary refresh)
    } else {
...
if (submitBtn && questionBox && answerModal && modalAnswerTextDiv && modalSourceInfoDiv && modalSourceListUl && loadingStateDiv && errorStateDiv && errorTextDiv && closeButton) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    console.error("Failed to parse error response body (might not be JSON):", parseError);
                    const rawErrorText = await response.text();
                    errorDetails += `, Raw Response: ${rawErrorText.substring(0, 100)}...`; 
                }
                ...
                if (result && result.error) {
                    ...
                    displayMainState('error');
                    console.error("Backend returned an error JSON:", result.error);
                    ...
                    if (result.used_context_metadata && result.used_context_metadata.length > 0) { //
                        
                        result.used_context_metadata.forEach(meta => { //
                            
                            if (meta.source_file && typeof meta.source_file === 'string') { //
                               
                                const filename = meta.source_file.replace('_cleaned.txt', '');
